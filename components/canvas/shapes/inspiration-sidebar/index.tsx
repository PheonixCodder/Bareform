...
export const InspirationSidebar = ({
  ...
  const clearAllImages = async () => {
    const imagesToRemove = images.filter(
      (img) => img.storageId && img.isFromServer && projectId,
    );

    const removePromises = imagesToRemove.map((image) =>
    ...
    try {
      ...
      console.error("Failed to clear some images:", error);
    }
  ...
  return (
    <div
      className={cn(
        "fixed left-0 top-1/2 transform -translate-y-1/2 w-80 backdrop-blur-xl bg-white/[0.08] border border-white/[0.12] gap-2 p-3 saturate-150 rounded-lg z-50 transition-transform duration-300",
