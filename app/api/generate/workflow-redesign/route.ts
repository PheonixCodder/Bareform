...
export async function POST(request: NextRequest) {
  try {
    ...
    const { userMessage, generatedUID, currentHTML, projectId } = body;

    if (!userMessage || !generatedUID || !currentHTML || !projectId) {
      return NextResponse.json(
        {
          error:
            "Missing required fields: userMessage, generatedUID, currentHTML, projectId",
        },
        {
          status: 400,
        },
    ...
    const styleGuideData = preloadedQueryResult(
