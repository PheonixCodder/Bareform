...
export const useWorkflowGeneration = () => {
  const dispatch = useAppDispatch();
  const [_, { isLoading: isGeneratingWorkflow }] =
  ...
  const generateWorkflow = async (generatedUIId: string) => {
    try {
      ...
      const workflowPromises = Array.from({ length: pageCount }).map(
        async (_, index) => {
          try {
            ...
            if (!response.ok) {
              throw new Error(`Failed to generate page ${index + 1}`);
            }
      ...
      if (successCount === pageCount) {
        toast.success(
          `👏 All ${pageCount} workflow pages generated successfully!`,
          {
            id: "workflow-generation",
          },
        );
        ...
        toast.success(
          `👏 Generated ${successCount}/${pageCount} workflow pages successfully!`,
          { id: "workflow-generation" },
        );
        if (failureCount > 0) {
...
export const useGlobalChat = () => {
  ...
  const [activeGeneratedUID, setActiveGeneratedUID] = useState<string | null>(
    null,
  );
  ...
  const exportDesign = async (
    ...
    if (!element) {
      console.warn("❌ No element to export for shape:", generatedUID);
      toast.error("No design element found for export.");
      return;
    }

    try {
      const filename = `generated-ui-${generatedUID.slice(0, 8)}.png`;
  ...
  const toggleChat = (generatedUIId: string) => {
    setActiveGeneratedUID(generatedUIId);
    setIsChatOpen(true);
  };

  const closeChat = () => {
    setIsChatOpen(false);
    setActiveGeneratedUID(null);
  };
...
export const useChatWindow = (generatedUID: string, isOpen: boolean) => {
  const [inputValue, setInputValue] = useState("");
  const scrollAreaRef = useRef<HTMLDivElement>(null);
  const inputRef = useRef<HTMLInputElement>(null);
  const dispatch = useAppDispatch();
  const chatState = useAppSelector((state) => state.chat.chats[generatedUID]);
  const currentShape = useAppSelector(
    (state) => state.shapes.shapes.entities[generatedUID],
  );
  ...
  useEffect(() => {
    if (isOpen) {
      dispatch(initializeChat(generatedUID));
    }
  }, [dispatch, generatedUID, isOpen]);
  ...
  const handleSendMessage = async () => {
    ...
    try {
      dispatch(
        addUserMessage({ generatedUIId: generatedUID, content: message }),
      );
      const responseId = `response-${Date.now()}`;
      dispatch(
        startStreamingResponse({
          generatedUIId: generatedUID,
      ...
      if (isWorkflowPage) {
        ...
        if (sourceFrame && sourceFrame.type === "frame") {
          ...
          console.error("No source frame available for wireframe snapshot");
        }
      }
  ...
  const handleClearChat = () => {
    dispatch(clearChat(generatedUID));
  };
