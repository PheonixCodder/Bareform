...
const grantKey = (
  ...
  return periodEndMs !== undefined && periodEndMs !== null
    ? `${subId}:${periodEndMs}`
...
export const handlePolarEvent = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    const incoming = event.data as PolarWebhookEvent<unknown>;
    ...
    const userId: Id<"users"> | null = await step.run(
      ...
      async () => {
        ...
        if (email) {
          try {
            console.log("⚙️ [Inngest] Looking up user by email:", email);
            ...
            console.error("⚠️ [Inngest] Email lookup failed for:", email);
            return null;
          }
        }
        console.error("❌ [Inngest] No email found to lookup for");
    ...
    const subscriptionId = await step.run("upsert-subscription", async () => {
      try {
        ...
        console.log(
          "🔍 [Inngest] Checking for existing subscriptions first...",
        );
        ...
        if (allUserSubs && allUserSubs.length > 1) {
          console.error("⚠️ [Inngest] MULTIPLE SUBSCRIPTIONS FOUND FOR USER:");
